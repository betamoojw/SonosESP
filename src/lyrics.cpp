...
void createLyricsOverlay(lv_obj_t* parent) {
    ...
    // Absolute position: x=30 matches img_album left edge (LEFT_MID+30 offset)
    // y=270 = 480(panel) - 180(container) - 30(art bottom margin)
    lv_obj_set_pos(lyrics_container, 30, 270);
...
void updateLyricsDisplay(int position_seconds) {
    ...
    // Show instantly — cancel any ongoing fade-out
    lyricsShow();
    ...
    // Snap text to new line (no fade-in — instant feel)
